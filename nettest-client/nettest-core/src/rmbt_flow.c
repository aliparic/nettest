	...
	for (rp = res; rp != NULL; rp = rp->ai_next) {
		...
		if (s->config->bind_ip != NULL) {
			...
			freeaddrinfo(res_bind);
                ...
                char optval[16];
                unsigned int optlen;
                strcpy(optval,s->config->tcp_cca);
                optlen=(unsigned int)strlen(optval);
                int r = setsockopt(sfd, 6, TCP_CONGESTION, optval, optlen);                
		...
		if (errno == EINPROGRESS) { // is nonblocking
			...
			if (poll_res > 0) { // socket ready
				...
				socklen_t err_len = sizeof(err);
			        r = getsockopt(sfd, SOL_SOCKET, SO_ERROR, &err, &err_len);
				if (r == -1)
...
static bool run_test(State *s) {
	...
	my_log(s, "connected with %" PRIuFAST16 " flow(s) for dl; %" PRIuFAST16 " flow(s) for ul", s->config->dl_num_flows, s->config->ul_num_flows);
        my_log(s, "TCP CONGESTION CHANGED for  %" PRIuFAST16 " flow(s) for dl; %" PRIuFAST16 " flow(s) for ul", s->config->dl_num_flows, s->config->ul_num_flows);        
	/* pretest downlink */
